...
# Convert all tables to 1NF

# Uber categories
u_categories = (
    ur[["id", "category"]]
    .dropna()
    .assign(category=lambda df: df["category"].str.split(","))
    .explode("category")
)

u_categories["category"] = u_categories["category"].str.strip().str.lower()
u_categories = u_categories.rename(columns={"id": "restaurant_id"})
u_categories = u_categories[["restaurant_id", "category"]].drop_duplicates()

# Yelp categories
y_categories = (
    yb[["business_id", "categories"]]
    .dropna()
...
y_categories = (
    y_categories[["business_id", "category"]]
    .drop_duplicates()
)
...
for _, row in yb[["business_id", "hours"]].dropna().iterrows():
    try:
        hours_dict = ast.literal_eval(row["hours"])

        for day, hrs in hours_dict.items():
            hours_rows.append([
                row["business_id"],
                day,
                hrs
...
y_hours = pd.DataFrame(
    hours_rows,
    columns=["business_id", "day_of_week", "hours"]
)
...
u_categories.to_csv("uber_restaurant_categories.csv", index=False)
y_categories.to_csv("yelp_restaurant_categories.csv", index=False)
y_hours.to_csv("yelp_restaurant_hours.csv", index=False)
...
files.download("uber_restaurant_categories.csv")
files.download("yelp_restaurant_categories.csv")
files.download("yelp_restaurant_hours.csv")
